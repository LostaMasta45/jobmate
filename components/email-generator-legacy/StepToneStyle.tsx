...
const OPENING_HOOKS = [
  {
    ...
    desc: 'Buka dengan pencapaian impressive',
    example: '"Dalam 6 bulan terakhir, saya berhasil meningkatkan conversion rate 40%..."',
  ...
  },
  {
    ...
    icon: '📖',
    label: 'Story Hook',
    desc: 'Cerita mini yang engaging',
    example: '"Saat pertama kali menggunakan produk Anda di 2020, saya tidak menyangka..."',
    color: 'text-blue-600',
  ...
  },
  {
    ...
    desc: 'Mutual connection atau shared interest',
    example: '"Setelah membaca artikel Bapak di LinkedIn tentang engineering culture..."',
    color: 'text-green-600',
  ...
  },
  {
    ...
    icon: '❓',
    label: 'Question Hook',
    desc: 'Pertanyaan yang thought-provoking',
  ...
  },
  {
    ...
    desc: 'Langsung to the point dengan confidence',
...
export function StepToneStyle({ formData, updateFormData }: StepToneStyleProps) {
  ...
  const applySuggestion = () => {
    if (suggestedTone) {
      ...
      const mappedTone = suggestedTone === 'semiformal' ? 'professional' : suggestedTone as 'formal' | 'professional' | 'casual';
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            <Label className="text-lg font-semibold">Gaya Pembuka Email</Label>
            <p className="text-xs text-muted-foreground">Pilih bagaimana email kamu dimulai untuk menarik perhatian HRD</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {OPENING_HOOKS.map((hook) => (
            <button
              ...
              className={cn(
                "p-4 rounded-xl border-2 text-left transition-all flex flex-col gap-2 hover:shadow-md",
                formData.openingStyle === hook.value
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {formData.openingStyle === hook.value && (
                  <div className={cn("w-2 h-2 rounded-full", hook.color.replace('text-', 'bg-'))} />
                )}
              </div>
              <div>
                <h4 className={cn(
                  "font-semibold text-sm",
                  formData.openingStyle === hook.value ? hook.color : "text-slate-700 dark:text-slate-300"
                )}>
                  {hook.label}
                </h4>
                <p className="text-[10px] text-muted-foreground mt-0.5">{hook.desc}</p>
              </div>
              {formData.openingStyle === hook.value && (
                <div className="text-[9px] italic text-muted-foreground bg-white/50 dark:bg-slate-900/50 p-2 rounded-lg mt-1 leading-relaxed">
                  {hook.example}
                </div>
              )}
            </button>
