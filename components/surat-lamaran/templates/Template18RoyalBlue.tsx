...
export function Template18RoyalBlue({ data }: { data: FormState }) {
  ...
  return (
    <div className="space-y-3" style={{ fontFamily: 'Calibri, sans-serif', fontSize: '11pt', lineHeight: '1.5', color: '#0f172a' }}>
      <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#1D4ED8', color: 'white', borderRadius: '6px', marginBottom: '12px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 3fr', gap: '8px', fontSize: '10pt', marginTop: '10px', marginBottom: '10px' }}>
        <div style={{ fontWeight: 'bold', color: '#1D4ED8' }}>Nama Lengkap</div><div>: {biodata.namaLengkap || "—"}</div>
        <div style={{ fontWeight: 'bold', color: '#1D4ED8' }}>Tempat/Tgl Lahir</div><div>: {biodata.tempatLahir || "—"}, {formatTanggalID(biodata.tanggalLahir)}</div>
        <div style={{ fontWeight: 'bold', color: '#1D4ED8' }}>Pendidikan</div><div>: {biodata.pendidikan || "—"}</div>
        <div style={{ fontWeight: 'bold', color: '#1D4ED8' }}>No. HP / Email</div><div>: {biodata.noHandphone || "—"} / {biodata.email || "—"}</div>
        <div style={{ fontWeight: 'bold', color: '#1D4ED8' }}>Alamat</div><div>: {biodata.alamatLengkap || "—"}</div>
      </div>
      ...
      {perusahaan.lampiran && perusahaan.lampiran.length > 0 && (
        <div style={{ marginTop: '10px', padding: '10px', border: '2px solid #BFDBFE', borderRadius: '4px' }}>
          <div style={{ fontWeight: 'bold', color: '#1D4ED8', marginBottom: '6px' }}>Lampiran Dokumen:</div>
