...
import type { WAThankYouFormData, WAGeneratedMessage } from "./types";
...
const INITIAL_DATA: WAThankYouFormData = {
    yourName: '',
    position: '',
    companyName: '',
    hrdName: '',
...
interface WAFormThankYouProps {
    userName?: string;
    onBack?: () => void;
}

export function WAFormThankYou({ userName = '', onBack }: WAFormThankYouProps) {
    const [formData, setFormData] = useState<WAThankYouFormData>({
        ...INITIAL_DATA,
        yourName: userName,
    });
    ...
    const updateFormData = (data: Partial<WAThankYouFormData>) => {
        ...
        try {
            const result = await generateWhatsAppMessage({
                ...
                previousInteraction: formData.interviewDate
                    ? `Interview on ${formData.interviewDate}`
                    : undefined,
                specificReason: formData.memorableMoment || formData.specificTakeaway,
    ...
    const handleSave = async () => {
        ...
        try {
            const result = await saveWAMessage({
                ...
                hrdPhone: formData.hrdPhone,
                previousInteraction: formData.interviewDate,
                specificReason: formData.memorableMoment,
                toneStyle: formData.toneStyle,
                personality: formData.personality,
                messageLength: formData.messageLength,
                useEmoji: formData.useEmoji,
    ...
    return (
        <div className="h-[calc(100vh-4rem)] flex flex-col overflow-hidden bg-background">
            <WALoadingOverlay isLoading={isGenerating} messageType="thank_you" />
                ...
                <div className="max-w-2xl mx-auto px-4 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            <h1 className="text-lg font-semibold">Pesan Terima Kasih</h1>
                            <p className="text-sm text-muted-foreground">
                                {showPreview ? "Preview & Kirim" : "Isi detail interview"}
                            </p>
                        </div>
                        <div className="flex gap-1">
                            <div className={cn("h-2 w-8 rounded-full", "bg-pink-500")} />
                            <div className={cn("h-2 w-8 rounded-full", showPreview ? "bg-pink-500" : "bg-muted")} />
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        {showPreview ? (
                            ...
                            >
                                ...
                                <div className="space-y-4 pt-4 border-t border-dashed">
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="memorableMoment">Momen Memorable (Opsional)</Label>
                                        <Textarea
                                            id="memorableMoment"
                                            value={formData.memorableMoment}
                                            onChange={(e) => updateFormData({ memorableMoment: e.target.value })}
                                            placeholder="Contoh: Diskusi tentang project expansion ke ASEAN"
                                            className="min-h-[80px] resize-none"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="specificTakeaway">Insight/Takeaway (Opsional)</Label>
                                        <Textarea
                                            id="specificTakeaway"
                                            value={formData.specificTakeaway}
                                            onChange={(e) => updateFormData({ specificTakeaway: e.target.value })}
                                            placeholder="Contoh: Insight tentang culture perusahaan yang collaborative"
                                            className="min-h-[80px] resize-none"
                                        />
                                    </div>
                                </div>
                                ...
                                {/* HRD Info */}
                                <div className="space-y-4 pt-4 border-t border-dashed">
                                    <Label className="text-base font-semibold">Info Interviewer (Opsional)</Label>
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            <Label htmlFor="hrdName">Nama Interviewer</Label>
                                            <Input
                                                id="hrdName"
                    ...
                    <div className="max-w-2xl mx-auto px-4 py-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <><Sparkles className="h-4 w-4 mr-2" /> Generate Pesan Terima Kasih</>
                            )}
                        </Button>
                    </div>
                </div>
