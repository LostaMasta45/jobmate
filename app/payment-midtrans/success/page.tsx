...
function SuccessPageContent() {
    ...
    const externalId = searchParams.get('external_id') || searchParams.get('order_id');
    ...
    // ============================================
    // MIDTRANS VERSION: Use Midtrans API endpoint
    // ============================================
    const fetchPaymentStatus = async (attempt: number = 0) => {
        if (!externalId) {
            console.warn('[Midtrans Success Page] No external_id/order_id provided');
            setLoading(false);
            return;
        }

        console.log(`[Midtrans Success Page] Fetching payment status (attempt ${attempt + 1}/${maxRetries})...`);
        setIsRetrying(attempt > 0);

        try {
            const response = await fetch(`/api/payment-midtrans/check-status?external_id=${externalId}`);
            console.log('[Midtrans Success Page] API Response status:', response.status);

            if (!response.ok) {
                if (response.status === 404 && attempt < maxRetries) {
                    console.log(`[Midtrans Success Page] Payment not found yet, will retry in 3 seconds... (${attempt + 1}/${maxRetries})`);
            ...
            console.log('[Midtrans Success Page] Payment data:', data.payment);
            ...
            if (data.success && data.payment) {
                ...
                setLoading(false);
                setIsRetrying(false);
                console.log('[Midtrans Success Page] Payment found successfully!', data.source ? `(source: ${data.source})` : '');
            } else {
                console.error('[Midtrans Success Page] Payment not found:', data.error);
                setLoading(false);
                setIsRetrying(false);
            }

        } catch (error: any) {
            console.error('[Midtrans Success Page] Error fetching payment:', error);

            if (attempt < maxRetries) {
                console.log(`[Midtrans Success Page] Error occurred, will retry in 3 seconds... (${attempt + 1}/${maxRetries})`);
                setRetryCount(attempt + 1);
                ...
                console.error('[Midtrans Success Page] Max retries exceeded, giving up');
                setLoading(false);
                setIsRetrying(false);
            }
        }
                ...
                <Card className="max-w-md w-full">
                    ...
                    <CardContent className="text-center space-y-4">
                        ...
                        <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg text-xs font-mono break-all">
                            {externalId || 'No order_id provided'}
                        </div>
                        ...
                        <Button asChild className="w-full">
                            <a href="/payment-midtrans">Kembali ke Halaman Payment</a>
                        </Button>
                    </CardContent>
                </Card>
            </div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 py-6 sm:py-12 px-3 sm:px-4 lg:px-6 relative overflow-hidden">
            ...
            <div className="container max-w-5xl mx-auto relative z-10">
                ...
                >
                    <Badge variant="outline" className="bg-blue-50 dark:bg-blue-950/50 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 px-4 py-1">
                        🔧 Midtrans Test Version
                    </Badge>
                </motion.div>
...
export default function PaymentMidtransSuccessPage() {
    return (
        <Suspense fallback={
